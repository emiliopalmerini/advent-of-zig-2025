...
const day1 = @import("advent_of_zig_2025").day1;

test "starting from 50 L68 return 82" {
    const start = 50;
    const ops = [_]i32{-68};
    try std.testing.expectEqual(@as(i32, 82), day1.turn(start, ops[0]));
}
...
test "example part 2 L68 from 50 crosses 0 once" {
    const start = 50;
    const op = -68;
    try std.testing.expectEqual(@as(i32, 1), day1.countZeroCrossings(start, op));
}

test "example part 2 L30 from 82 does not cross 0" {
    const start = 82;
    const op = -30;
    try std.testing.expectEqual(@as(i32, 0), day1.countZeroCrossings(start, op));
}
...
test "example part 2 complete returns 5" {
    ...
    var res: i32 = 0;
    for (ops) |value| {
        res += day1.countZeroCrossings(start, value);
        start = day1.turn(start, value);
    }
